...
# Adding validation rules for age and Investment Amount
...
def validation(age, investment_amount):
    if age is not None and (int(age) < 0 or int(age) > 65):
        return build_validation_result(
            False,
            "age",
            "You should be between the ages of 0 and 65 to use this service, please update your age to be eligable.",
        )
    if investment_amount is not None and (int(investment_amount) < 5000):
        return build_validation_result(
            False,
            "investmentAmount",
            "The minimum investment is $5000, please increase the invetment amount!",
        )
...
def recommend_portfolio(intent_request):
    ...
    if source == "DialogCodeHook":
        # quickly validate the user inputs
        # Use the elicit slot type response
        
        build_bot = get_slots(intent_request)
    ...
    # get current session attibutes
        output_session_attributes = intent_request["sessionAttributes"]

        return delegate(output_session_attributes, get_slots(intent_request))

    # Get the investment recommendation based on risk level
    ...
    # Give an brief message of the investment recommendation based on the risk level
    return close(
        intent_request["sessionAttributes"],
        "Fulfilled",
        {   "contentType": "PlainText",
            ...
            """.format(first_name, investment_recommendation),
        },
    )
